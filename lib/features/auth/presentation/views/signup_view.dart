...
class _SignupViewState extends State<SignupView> {
  ...
  UserRole get _role => sl<SessionManager>().role;

  void _onSignUp() {
    if (!_agreeToTerms) {
      FeedbackHandler.error(AppStrings.authTermsRequired.tr());
      return;
    }
    if (_formKey.currentState?.validate() ?? false) {
      final cubit = context.read<AuthRegisterCubit>();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: BlocListener<AuthRegisterCubit, AuthRegisterState>(
          listener: (context, state) {
            if (state is AuthRegisterError) {
              FeedbackHandler.error(state.message);
            } else if (state is AuthRegisterSuccess) {
              FeedbackHandler.success(AppStrings.authSignupSuccess.tr());
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SignupForm(
                      ...
                      medicalIdController: _medicalIdController,
